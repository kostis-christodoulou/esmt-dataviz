...
# to check whethr you see "blue" go to https://ismy.blue/
...
# 2. Scatterplots: mapping group to colour aesthetic ----
...
# 2.b) Default colour scheme: map group variable to colour
# this uses the default ggplot colour paletter
ggplot(toy_data, aes(x = x, y = y, colour = group)) +
  geom_point(size = 5)

# 2.c) Change colour palette from default to "Set2": No assumed ordering to group
ggplot(toy_data, aes(x = x, y = y, colour = group)) +
  geom_point(size = 5) +
  scale_colour_brewer(palette = "Set2")

# scale_colour_brewer palettes that are available for use with these scales:
#   
# Diverging Scale
...
# 2.d) Change colour palette from default to "Blues": Assumed ordering to group
ggplot(toy_data, aes(x = x, y = y, colour = group)) +
  geom_point(size = 5) +
...
# 3.c) The colour of bars corresponds to fill aesthetic
ggplot(toy_data, aes(x = group, y = count, fill = group)) +
  geom_col()

# 3.d) Change colour palette from default to "Set2": No assumed ordering to group
ggplot(toy_data, aes(x = group, y = count, fill = group)) +
  geom_col() +
  scale_fill_brewer(palette = "Set1")

# 3.e) Change colour palette from default to "YlOrBr": Assumed ordering to group
ggplot(toy_data, aes(x = group, y = count, fill = group)) +
  geom_col() +
